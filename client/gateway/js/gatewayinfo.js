...
function booksubscribe(){
	...
	asksbook=remote.book({currency_gets:goods.currency, issuer_gets: goods.issuer, currency_pays:money.currency, issuer_pays:money.issuer});
	bidsbook=remote.book({currency_gets:money.currency, issuer_gets:money.issuer,currency_pays: goods.currency, issuer_pays:goods.issuer});
	asksbook.on("model", function(offers){receiveOffers("asks",offers,"auto")});
	bidsbook.on("model", function(offers){receiveOffers("bids",offers,"auto")});
	///////////////////////////
	asksbook.on("trade", function(tradePays,tradeGets){tradeListener("asks", tradePays, tradeGets)});
	bidsbook.on("trade", function(tradePays,tradeGets){tradeListener("bids", tradePays, tradeGets)});
	//////////////////////////////////////////////////
	asksbook.on("offer_added", function(offer){offerListener("asks",offer,"added")});
	bidsbook.on("offer_added", function(offer){offerListener("bids",offer,"added")});
	////////////////////////////////
	asksbook.on("offer_removed", function(offer){offerListener("asks",offer,"removed")});
	bidsbook.on("offer_removed", function(offer){offerListener("bids",offer,"removed")});
}
...
function loadoffers(callback){
	...
	remote.request('book_offers', {taker_gets: money,
		taker_pays: goods})
		.request(function(err, res) {
			if(!err){
	...
	setInterval(function(){if((new Date() - lastfreshdate)>180000) loadoffers();}, 30000);//超过3分钟强制刷新数据，解决自动刷新意外终止问题
